...
/* Definição da estrutura de dados para threads */
typedef struct {
    int i_start;
    int i_end;
    int n;
...
void* thread_nussinov(void *arg) {
    thread_data_t *data = (thread_data_t*)arg;

    int i, j, k;
    for (i = data->i_start; i > data->i_end; i--) {
        for (j = i + 1; j < data->n; j++) {
            // ... O código original do kernel aqui ...
        }
...
int main(int argc, char** argv)
{
  ...
  polybench_start_instruments;
    ...
    for (int t = 0; t < num_threads; t++) {
        thread_data[t].i_start = N - (t * chunk_size);
        thread_data[t].i_end = N - ((t + 1) * chunk_size) - 1;
        thread_data[t].n = n;
        thread_data[t].seq = POLYBENCH_ARRAY(seq);
        thread_data[t].table = POLYBENCH_ARRAY(table);
        pthread_create(&threads[t], NULL, thread_nussinov, &thread_data[t]);
    }
